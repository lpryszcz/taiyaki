...
def get_loss(outputs, seqs, seqlens):
    """Return lossvector from model prediction reporting 2*bases output.
    Lossvector would be simple hamming distance to seqs."""
    nbatch = len(seqlens)
    print(nbatch, outputs.shape, seqs.shape); print(seqs[:seqlens[0]], outputs[:,0])#; 
    loss = np.zeros((nbatch, ), dtype=np.float32)
    for i in range(nbatch):
        preds = np.argmax(outputs[:,i]); print(i, seqs[:seqlens[i]], preds)
...
if __name__ == '__main__':
    ...
    model_kwargs = {
        ...
        'outsize': 2*nbase #flipflopfings.nstate_flipflop(nbase)
    }
    ...
    for i in range(args.niteration):
        ...
        # calculate loss
        lossvector = get_loss(outputs, seqs, seqlens) #ctc.crf_flipflop_loss(outputs, seqs, seqlens, 1.0)
