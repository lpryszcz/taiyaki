...
def _load_data(args, log, test_ref="cc6m_2244_T7_ecorv"):
    ...
    log.write('* Loading test and training...\n')
    read_ids, read_ids_test = [], []
    for l in open(args.read_ref):
        if not l.strip(): continue
        read_id, ref = l[:-1].split('\t')[:2]
        if ref==test_ref:
            read_ids_test.append(read_id)
        else:
            read_ids.append(read_id)
    log.write('*  {} training and {} testing reads loaded\n'.format(len(read_ids), len(read_ids_test)))

    if args.limit is not None:
        log.write('* Limiting number of strands to {}\n'.format(args.limit))

    # load training and training
    with mapped_signal_files.HDF5Reader(args.input) as per_read_file:
        ...
        read_data_test = per_read_file.get_multiple_reads(
            read_ids_test, max_reads=args.limit)
    ...
    return read_data, read_data_test, alphabet_info
...
def main():
    ...
    read_data, read_data_test, alphabet_info =  _load_data(args, log)
    ...
    for i in range(args.niteration):
        ...
        if (i + 1) % args.save_every == 0:
            ...
            loss = calculate_loss_on_testing(network, optimizer, read_data_test, batch_size, batch_chunk_len, filter_parameters, args,
