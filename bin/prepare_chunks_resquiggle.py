...
def resquiggle_many(fasta, fast5_reads, references, per_read_params_dict, alphabet_info, rna=1):
    ...
    for i, (filename, read_id) in enumerate(fast5_reads, 1):
        ...
        sig = (sig.current - read_params_dict['shift']) / read_params_dict['scale']
        yield refseq, sig, segs
...
def main():
    ...
    resquiggle = resquiggle_many(args.fasta, fast5_reads, references, per_read_params_dict, alphabet_info)

    # generate chunks.hdf5 and chunks.fa
    out = open(args.output[:-5]+".fa", "w")
    i = 0
    chunklen = 200 # this should be roughly 5 bases (75 bps @ 3kHz)
    #chunkstep = int(chunklen/2)
    chunks = [] #np.array()
    for refseq, sig, segs in resquiggle:
        # strip sig to seq only 
        sig = sig[segs[0]:segs[-1]]
        # make its length to be multiplication of chunklen and reshape into chunks
        rows = int(len(sig)/chunklen)
        sig = sig[:rows*chunklen].reshape((rows, chunklen))
        # store fasta
        rows_to_remove = []
        for rowi, s in enumerate(np.arange(segs[0], segs[0]+rows*chunklen, chunklen)):
            # that's not the most efficient
            seqpos = np.argwhere(np.all((segs>s, segs<=s+chunklen), axis=0)).flatten()
            if not len(seqpos):
                rows_to_remove.append(rowi)
                continue
            # get seq start & end; include previous base that starts before
            ss, se = seqpos[0]-1, seqpos[-1]
            out.write(">%s\n%s\n"%(i, refseq[ss:se]))
            i += 1
        # remove chunks that produce not seq
        if rows_to_remove:
            sig = np.delete(sig, rows_to_remove, axis=0)
        # store chunks
        if not len(chunks):
            chunks = sig
        else:
            chunks = np.vstack((chunks, sig))
    print("Saving chunks %s to %s"%(chunks.shape, args.output))
    # store chunks in hdf5
    h5 = h5py.File(args.output, "w")
    # store in chunks - use gzip compression, 3-4x size reduction ie 39Mb vs 12 Mb with gzip
    dset = h5.create_dataset("chunks", data=chunks, compression="gzip", 
                             chunks=(int(np.ceil(100000/chunklen)), chunklen))
    h5.close()
