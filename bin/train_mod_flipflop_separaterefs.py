...
parser.add_argument(
    '--read_ref', required=True, #default="", metavar='samples', type=Positive(int),
    help='File that assign reads to references')
parser.add_argument(
...
def _load_data(args, log):
    ...
    log.write('* Loading read ids...\n')
    read_data, ref2read_ids = [], {}
    for l in open(args.read_ref):
        if not l.strip(): continue
        read_id, ref = l[:-1].split('\t')[:2]
        if ref not in ref2read_ids:
            ref2read_ids[ref] = []
        ref2read_ids[ref].append(read_id)
    log.write('*  {} reads from {} references loaded\n'.format(sum(map(len, ref2read_ids.values())), len(ref2read_ids)))

    if args.limit is not None:
        log.write('* Limiting number of strands to {}\n'.format(args.limit))

    # load reads separately for every reference
    with mapped_signal_files.HDF5Reader(args.input) as per_read_file:
        (bases_alphabet, collapse_alphabet, mod_long_names) = per_read_file.get_alphabet_information()
        for ref, read_ids in ref2read_ids.items():
              read_data.append(per_read_file.get_multiple_reads(read_ids, max_reads=args.limit))
...
def main():
    ...
    ref_read_data, alphabet_info =  _load_data(args, log)
    refi = 0 # to keep track of references used
    read_data = ref_read_data[refi%len(ref_read_data)] # get read_data from first reference
    ...
    for i in range(args.niteration):
        ...
        loss_log.write('{}\t{:.10f}\t{:.10f}\n'.format(i, fval, score_smoothed.value))
        if (i + 1) % args.save_every == 0:
            refi += 1 # update refi and change to read_data from next reference
            read_data = ref_read_data[refi%len(ref_read_data)]
            # calculate loss on test data
            loss = calculate_loss_on_testing(network, optimizer, read_data, batch_size, batch_chunk_len, filter_parameters, args,
                                         log, chunk_log, target_batch_size, device, 
                                         flipflop_can_labels, flipflop_mod_labels, flipflop_ncan_base, 
                                         can_mods_offsets, mod_cat_weights, mod_factor_t)
            fval = float(loss)
            score_smoothed.update(fval)
            del loss
...
def calculate_loss_on_testing(network, optimizer, read_data, batch_size, batch_chunk_len, filter_parameters, args,
                              log, chunk_log, target_batch_size, device, 
                              flipflop_can_labels, flipflop_mod_labels, flipflop_ncan_base, 
                              can_mods_offsets, mod_cat_weights, mod_factor_t):
    ...
    loss.backward()
    optimizer.step()
    del indata, seqs, mod_cats, seqlens, outputs, lossvector
    return loss
